...
namespace BBUnity.Conditions
{
    ...
    /// </summary>
    [Condition("Perception/CustomIsCybotClose")]
    [Help("Checks whether a target is close depending on a given distance")]
    public class CustomIsCybotClose : GOCondition
    {
        ...
        {
            GameObject[] cybots = GameObject.FindGameObjectsWithTag("NPC");
            for(int i = 0; i < cybots.Length; i++)
            {
                GameObject cy = cybots[i];
                if(gameObject != cy && (gameObject.transform.position - cy.transform.position).sqrMagnitude < closeDistance * closeDistance)
                {
                    gameObject.GetComponent<NPCScript>().toChase = cy;
                    return true;
                }
            }
            return false;
